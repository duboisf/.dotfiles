...
local function files_in_last_commit()
  Snacks.picker({
    title = "Files in Last Commit",
    finder = "proc",
    cmd = "git",
    ...
    preview = function(ctx)
      local cmd = { "git", "diff", "HEAD~1..HEAD", "--", ctx.item.file }
...
return {
  ...
  keys = {
    ...
    { "<leader>fs",      function() Snacks.picker.git_status() end,                               desc = "Find files in directory of current file" },
    ...
    { "<leader>gc",      files_in_last_commit,                                                    desc = "Files in last commit" },
